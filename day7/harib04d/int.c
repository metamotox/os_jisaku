...
void inthandler21(int *esp)
{
	...
	unsigned char data;
	io_out8(PIC0_OCW2, 0x61);		/* inform PIC : IRQ-01 Reception completed */
	...
	if(keybuf.len < 32){
		...
		keybuf.len++;
		keybuf.next_w++;
		/* ring buffer size is 32 */
		/* so, if next_w position of array is 32, */
		/* set next_w 0 (set position first of array */
		if(keybuf.next_w == 32){
			keybuf.next_w = 0;
		}
		return;
